...
class ControllerUsuario extends Controller
{
	...
	public function listar(){
		echo "entrou em listar alunos <br>";
		$objeto_model = new ModelUsuario();
	...
	public function form(){
		...
		$data['alunos'] = $execute; //o array dados tem o indice 'alunos'. isso vai ser tratado em listagem.html no foreach..
		$view->data = $data;
		$view->carregar("usuario/cadastro.html");
		$view->mostrar();
	}

	public function editUserDocs($parametros){
		echo "funcao editUserDocs() no controller <br>";

		$id = $parametros[0];

		echo "id alterar editUserDocs:" .$id;
		$objeto_model = new ModelUsuario();
		...
		$view->data = $data;
		$view->carregar("usuario/userDocs.html");
		$view->mostrar();
	}


	public function editUserOferta($parametros){
		...
		$data['retorno_alunos'] = $execute;
		$view->data = $data;
		$view->carregar("usuario/userOferta.html");
		$view->mostrar();
	}

	public function editDataCourse($parametros){
		echo "funcao alterar() no controller <br>";
		...
		$data['retorno_alunos'] = $execute;
		$view->data = $data;
		$view->carregar("usuario/userDataCourse.html");
		$view->mostrar();
	}
	...
	public function update($parametros){
		...
		// echo "chamou a função update dentro de controllerUsuario";

		$id = $parametros[0];
		$id = $_POST['id'];
		echo "id em update: " .$id. "<br>";

		$objeto_model = new ModelUsuario();
		$execute = $objeto_model->alterUserDocs($id);
	}

	public function updateUserOferta($parametros){

		//echo "chamou a função update dentro de controllerUsuario";
		$id = $_POST['id'];
		$objeto_model = new ModelUsuario();
		$execute = $objeto_model->alterUserOferta($id);
	}

	public function updateUserCourse($parametros){

		//echo "chamou a função update dentro de controllerUsuario";
		$id = $_POST['id'];
		$objeto_model = new ModelUsuario();
		$execute = $objeto_model->alterUserCourse($id);
	}
